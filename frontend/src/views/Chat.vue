...
function connectWebSocket() {
  ws.value = new WebSocket(`${WS_URL}/ws/${route.params.roomId}`)

  ws.value.onmessage = (event) => {
    const data = JSON.parse(event.data)
    messages.value.push(data)

    if (data.username !== userStore.username) {
...
<template>
  <div class="min-h-screen bg-gray-100 p-4 md:p-8">
    <div class="max-w-5xl mx-auto h-[calc(100vh-4rem)] md:h-[calc(100vh-8rem)] flex flex-col bg-white rounded-2xl shadow-2xl overflow-hidden">
    ...
    <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
      <div v-for="(msg, index) in messages" :key="index" class="flex" :class="msg.username === userStore.username ? 'justify-end' : 'justify-start'">
